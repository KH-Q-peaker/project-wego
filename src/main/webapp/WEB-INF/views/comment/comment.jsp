...
<script type="text/javascript" src="${path}/resources/js/comment.js"
	defer></script>
<script type="text/javascript" src="${path}/resources/js/delete.js"
	defer></script>
<script type="text/javascript" src="${path}/resources/js/report.js"
	defer></script>
...
<div class="cmtcontainer">
	...
	<div class="cmtwrite" id="cmtwrite">
		<textarea id="contents" name="contents" placeholder="댓글을 작성해주세요."
		...
		<c:if test="${c.status != 'Y' }">
			<div class="comments ${c.mentionId == null ? '' : 'mention'}">
				...
				<c:if test="${c.status == 'N' }">
					...
					<a class="cmtuser"
						href="http://localhost:8080/profile/${c.userId }">${c.nickname}</a>
					<div class="cmtdate">
						<fmt:formatDate pattern="MM-dd HH:mm"
							value="${c.modifiedDt == null? c.createdDt : c.modifiedDt}"></fmt:formatDate>
					...
					<div class="btns">
						<input type="hidden" id="commentId" name="commentId"
							value="${c.commentId}">
						<c:if
							test="${c.userId == sessionScope.__AUTH__.userId && c.reportCnt < 5 }">
							<input type="button" class="modifycmt" name="modifycmt"
								value="수정" />
							<input type="button" class="deletecmt" name="deletecmt"
								value="삭제" />
						</c:if>
				...
				<c:choose>
					...
					<c:otherwise>
						...
						<c:if test="${c.mentionId == null}">
							<input type="button" class="mentionbtn" name="mentionbtn"
								value="↪답글" />
							<c:if test="${c.mentionCnt != 0 }">
								<span class="mentionCnt">답글 <span id="mentionCnt">${c.mentionCnt }</span>개
								</span>
							</c:if>
						</c:if>
					</c:otherwise>
				...
				<div class="updatebtn">
					<input type="button" name="updatecls" value="취소" /> <input
						type="button" name="updatecmt" value="수정" />
				</div>
			</div>
			...
			<c:if test="${c.mentionId == null}">
				<div class="cmtwrite mentionwrite">
					<input type="hidden" id="mentionId" name="mentionId"
						value="${c.commentId}">
					<textarea id="mcontents" class="mcontents" name="contents"
						placeholder="답글을 작성해주세요." maxlength="1000" required></textarea>
