		...
		<tbody>
			<c:forEach items="${availableParty}" var="YesParty">
				<tr>
					...
					<td class="t2"><a href="/party/${YesParty.sanPartyId}">
							${YesParty.title}</a></td>
					<td class="t3"><fmt:formatDate pattern="yyyy-MM-dd"
							value="${YesParty.partyDt}"></fmt:formatDate></td>
		...
		<ul class="paginUl">
			...
			<c:forEach var="pageNum" begin="${availPage.startPage}"
				...
				<li
					...
					onclick="selectClickAvailableCurrPage()"><span
					data-temp=" ${availPage.cri.setCurrPage(pageNum)}">${pageNum}</span>
					<input type="hidden" id="availcurrPage" value="${__aCurrPage__}">
				</li>
			</c:forEach>
			...
			<c:if test="${availPage.next}">
				...
				<li class="next"><span
					...
					type="hidden" id="availcurrPageNext" value="${availPage.endPage+1}"></li>
			</c:if>
		</ul>
	</div>
</div>
