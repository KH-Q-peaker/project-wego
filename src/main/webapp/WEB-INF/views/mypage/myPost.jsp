			...
			<c:forEach items="${writtenList}" var="profileVO">
				<tr>
					...
					<td class="t3"><fmt:formatDate pattern="yyyy-MM-dd"
							value="${profileVO.createDt}"></fmt:formatDate></td>
		...
		<form action="paginationForm">
			<ul class="paginUl">
				...
				<c:forEach var="pageNum" begin="${pageMaker.startPage}"
					...
					<li
						class="${param.currPage eq pageNum ? 'currPage' : ''}  myPostPage${pageNum}"
						data-mypost-page="${__MyPostCurrPage__}" id="postCurrPageNum"
						onclick="selectClickCurrPage()"><span
						data-temp="${pageMaker.cri.setCurrPage(pageNum)}" id="currPageNum">${pageNum}</span>
					</li>
					<!-- 숫자만 표시됨. -->
				</c:forEach>
