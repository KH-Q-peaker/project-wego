...
<link rel="shortcut icon" href="${path}/resources/ico/favicon.ico"
	type="image/x-icon" />
<link rel="icon" href="${path}/resources/ico/favicon.ico"
	type="image/x-icon" />
...
<link rel="stylesheet" type="text/css"
	href="${path}/resources/css/like.css" />
<link rel="stylesheet" type="text/css"
	href="${path}/resources/css/comment.css" />
<link rel="stylesheet" type="text/css"
	href="${path}/resources/css/party-detail.css" />

<script type="text/javascript" src="${path}/resources/js/header.js"
	defer></script>
<script type="text/javascript" src="${path}/resources/js/footer.js"
	defer></script>
<script type="text/javascript" src="${path}/resources/js/default.js"
	defer></script>
<script type="text/javascript" src="${path}/resources/js/like.js" defer></script>
<script type="text/javascript" src="${path}/resources/js/scroll.js"
	defer></script>
<script type="text/javascript" src="${path}/resources/js/comment.js"
	defer></script>
<script type="text/javascript"
	src="${path}/resources/js/party-detail.js" defer></script>
...
<body>
	<div class="total-wrap">
		...
		<section>
			...
			<div class="contents">
				<img src=" ${party.userPic }" alt="" class="userpic" /> <a
					class="username"
					href="http://localhost:8080/profile/${party.userId}">${party.nickName}</a>
				<div class="likeCnt">
				...
				<div class="partyInfo">
					<div class="info">
						<span class="list">날짜</span> <span><fmt:formatDate
								pattern="yy년 MM월 dd일" value="${party.partyDt}"></fmt:formatDate></span>
					</div>
					<div class="info">
						<span class="list">시간</span> <span><fmt:formatDate
								pattern="HH시 mm분" value="${party.partyDt}"></fmt:formatDate></span>
					</div>
					<div class="info">
						<span class="list">참여인원</span> <span id="currentCount">${party.userCnt}</span><span>/
							${party.partyMax }</span>
					</div>
				...
				<c:choose>
					<c:when test="${party.userCnt >= party.partyMax }">
						<input type="button" class="join"
							style="background-color: #727272" disabled value="모집완료" />
					</c:when>
					<c:when test="${isJoin == false }">
						<input type="button" class="join" id="join" name="join"
							value="참여하기" />
					</c:when>
					<c:otherwise>
						<input type="button" class="join" id="clsjoin" name="join"
							value="취소하기" />
					</c:otherwise>
				</c:choose>
			</div>
