...
<script src="/resources/js/user-posts.js" defer></script>
				...
				<ul class="paginUl">
					...
					<c:if test="${pageMaker.prev}">
						<li class="prev"><span
							...
							onclick="selectClickCurrPagePrev()">Prev</span> <input
					...
					<c:forEach var="pageNum" begin="${pageMaker.startPage}"
						...
						<li
							class="${param.currPage eq pageNum ? 'currPage' : ''} userPostsPage${pageNum}"
							data-curr-page="${__UserPostsCurrPage__}" id="currPageNum"
							onclick="selectClickCurrPage()"><span>${pageNum}</span></li>
						<!-- 숫자만 표시됨. -->
					</c:forEach>
	          ...
	          현재페이지 기준으로 계산되어야함. 다 모델속성에 PageMaker저장되어잇음-->
					<c:if test="${pageMaker.next}">
						<li class="next"><span
							...
							onclick="selectClickCurrPageNext()">Next</span> <input
