...
public class PPageDTO {
	...
	private PCriteria pcri;	//기준 정보는 기본으로 주어짐
	...
	public PPageDTO(PCriteria pcri, int totalAmount) {//두가지 정보로 아래를 계산함.
		...
		//----------------------------------------------------------//
		int currPage = pcri.getPCurrPage();
		int amount = pcri.getPAmount();
		int pagesPerPage = pcri.getPagesPerPage();
